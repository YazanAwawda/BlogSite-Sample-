...
export class NewBlogComponent implements OnInit {
  ...
  onSubmit() {
    ...
    if (this.addBlogForm.valid) {
      ...
      this.http.post('https://localhost:7265/api/Post', formData).subscribe(
        data => {
          console.log(data);
        }
      );
  ...
  get Title() {
    return this.addBlogForm.get('Title') as UntypedFormControl;
  }

  get Description() {
    return this.addBlogForm.get('Description') as UntypedFormControl;
  }

  get Banner_Image() {
    return this.addBlogForm.get('Banner_Image') as UntypedFormControl;
  }
